...
namespace
{
	...
	struct FFbxStaticCachePaths
	{
		FString MeshBinPath;
		FString MaterialBinPath;
	};
	struct FFbxSkeletalCachePaths
	{
		FString MeshBinPath;
		FString MaterialBinPath;
	};

	FFbxStaticCachePaths GetStaticCachePaths(const FString& NormalizedPath)
	{
		FString CachePathStr = ConvertDataPathToCachePath(NormalizedPath);
		return { CachePathStr + ".bin", CachePathStr + ".mat.bin" };
	}
	FFbxSkeletalCachePaths GetSkeletalCachePaths(const FString& NormalizedPath)
	{
		FString CachePathStr = ConvertDataPathToCachePath(NormalizedPath);
	...
	bool TryLoadStaticMeshCache(const FString& MeshBinPath, const FString& MatBinPath, FStaticMesh* Mesh, TArray<FMaterialInfo>& MaterialInfos)
	{
		try
		{
			...
			if (!Reader.IsOpen())
			{
				throw std::runtime_error("Failed to open FBX mesh cache file for reading.");
			}
			Reader << *Mesh;
			Reader.Close();
			...
			if (!MatReader.IsOpen())
			{
				throw std::runtime_error("Failed to open FBX material cache file for reading.");
			}
	...
	struct FSkeletonRebuildResult
	{
		USkeleton* Skeleton = nullptr;
		TArray<UBone*> BonePointers;
	};

	void GatherBonesRecursive(UBone* Bone, int32 ParentIndex, TArray<FSkeletalCacheBone>& OutBones, TMap<UBone*, int32>& OutBoneIndexMap)
	{
		if (!Bone)
		{
			return;
	...
	void BuildSkeletalCacheData(const FSkeletalMesh* Mesh, FSkeletalCacheData& OutData)
	{
		if (!Mesh)
		{
			return;
		...
		for (size_t FleshIdx = 0; FleshIdx < Mesh->Fleshes.size(); ++FleshIdx)
		{
			...
			CachedFlesh.BoneIndices.reserve(Flesh.Bones.Num());
			for (UBone* Bone : Flesh.Bones)
			{
				if (int32* BoneIndex = BoneIndexMap.Find(Bone))
				{
					CachedFlesh.BoneIndices.push_back(*BoneIndex);
				}
				else
				{
					CachedFlesh.BoneIndices.push_back(-1);
				}
			}
	...
	FSkeletonRebuildResult RebuildSkeletonFromCache(const TArray<FSkeletalCacheBone>& CachedBones)
	{
		FSkeletonRebuildResult Result;
		if (CachedBones.empty())
		{
			return Result;
		}

		Result.BonePointers.resize(CachedBones.size());
		for (size_t Index = 0; Index < CachedBones.size(); ++Index)
		{
			const FSkeletalCacheBone& CachedBone = CachedBones[Index];
			FName BoneName(CachedBone.Name.c_str());
			UBone* Bone = new UBone(BoneName, CachedBone.RelativeTransform);
			ObjectFactory::AddToGUObjectArray(UBone::StaticClass(), Bone);
			Bone->SetRelativeBindPoseTransform(CachedBone.BindPose);
			Result.BonePointers[Index] = Bone;
		}

		for (size_t Index = 0; Index < CachedBones.size(); ++Index)
		{
			const int32 ParentIndex = CachedBones[Index].ParentIndex;
			if (ParentIndex >= 0 && ParentIndex < static_cast<int32>(CachedBones.size()))
			{
				UBone* Parent = Result.BonePointers[ParentIndex];
				UBone* Child = Result.BonePointers[Index];
		...
		for (size_t Index = 0; Index < CachedBones.size(); ++Index)
		{
			if (CachedBones[Index].ParentIndex < 0)
			{
				Root = Result.BonePointers[Index];
				break;
			}
		}
		...
		if (!Root)
		{
			Root = Result.BonePointers.front();
		}

		Result.Skeleton = NewObject<USkeleton>();
		Result.Skeleton->SetRoot(Root);
		return Result;
	}

	// 스켈레탈 bin 내부 데이터가 정상 역 직렬화 된 경우 true 반환 
	bool TryLoadSkeletalMeshCache(const FString& MeshBinPath, const FString& MatBinPath, FSkeletalMesh* Mesh, TArray<FMaterialInfo>& MaterialInfos)
	{
		try
		{
			...
			if (!Reader.IsOpen())
			{
				throw std::runtime_error("Failed to open FBX skeletal mesh cache file for reading.");
			}
			...
			if (!MatReader.IsOpen())
			{
				throw std::runtime_error("Failed to open FBX skeletal material cache file for reading.");
			}
			Serialization::ReadArray(MatReader, MaterialInfos);
			MatReader.Close();
			...
			{
				FSkeletonRebuildResult RebuildResult = RebuildSkeletonFromCache(CacheData.Bones);
				Mesh->Skeleton = RebuildResult.Skeleton;
				...
				for (const FSkeletalCacheFlesh& CachedFlesh : CacheData.Fleshes)
				{
					...
					{
						if (BoneIndex >= 0 && BoneIndex < RebuildResult.BonePointers.Num())
						{
							Flesh.Bones.Add(RebuildResult.BonePointers[BoneIndex]);
						}
					}
...
#ifdef USE_OBJ_CACHE
	{
		FFbxSkeletalCachePaths CachePaths = GetSkeletalCachePaths(NormalizedPathStr);
...
#ifdef USE_OBJ_CACHE
	FFbxStaticCachePaths CachePaths = GetStaticCachePaths(Path);
	EnsureCacheDirectory(CachePaths.MeshBinPath);
...
#ifdef USE_OBJ_CACHE
	FFbxStaticCachePaths CachePaths = GetStaticCachePaths(NormalizedPathStr);
	EnsureCacheDirectory(CachePaths.MeshBinPath);
